...
El frontend actual cubre el shell, autenticación, consulta/creación de procesos y ejecuciones, el flujo operativo principal de Transformación Excel y el flujo de Conciliación Excel hasta la revisión manual de resultados. La revisión usa `PATCH /conciliaciones/resultados/{resultado_id}/revision`, exige `expected_updated_at` como precondición optimista y recupera resultados y progreso persistidos desde backend al volver a abrir el workspace. Los endpoints de resultados y revisión aíslan por `cliente_id`; una versión obsoleta o una ejecución que ya no está en `REQUIERE_REVISION` responde `409`. Un cambio material del mapping elimina transaccionalmente los resultados y el resumen anteriores para exigir una nueva ejecución.
